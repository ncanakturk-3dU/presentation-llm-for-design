...
- Deck JSONs live in **`content/`**. The **active** deck is the one imported by `src/lib/useContent.js` — by default `content/sample.json`, a placeholder deck with one of every slide type.
- To use a different deck, point that single import at another file in `content/`, or edit `content/sample.json`.
...
You can also edit the JSON by hand — copy the shape of any item in `content/sample.json`.
...
- `content/` — deck JSON files (the data).
- `src/slides/` — the eight slide layout components + shared helpers.
- `src/components/` — the Contents drawer and the cover sphere.
