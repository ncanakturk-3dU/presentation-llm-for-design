...
5. **Leave it unpresented unless asked.** Dropping the JSON into `content/` is enough to review it: designlab picks up every deck there and the screen's `deck` param walks them. Only when the user says this deck is the one they are presenting, set `DEFAULT_DECK` in `src/lib/decks.js` to its id — the app shows that deck and nothing else, so changing it changes what gets published. Leave older decks in `content/` as versions. Never overwrite `content/sample.json`: it is the reference deck, one slide per archetype, what the next deck starts from, and the source of the `Slide` archetype variants in the lab.
